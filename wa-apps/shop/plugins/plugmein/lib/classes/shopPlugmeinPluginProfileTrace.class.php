...
class shopPlugmeinPluginProfileTrace implements \Tracy\IBarPanel
{
    ...
    {
        $this->config = array_replace_recursive(self::getDefaultConfig(), $config);
    ...
    public function getTab()
    {
        ...
        return sprintf(
            ...
            Profiler::isEnabled() ? sprintf(
                $countOfProfiles == 1 ? "%d hook" : "%d hooks",
                $countOfProfiles
            ) : "disabled"
        );
    }
    ...
    public function getPanel()
    {
        ...
        $this->profilerService->iterateProfiles(function (Profile $profile) use (&$table, $log_events) {
            /** @noinspection PhpInternalEntityUsedInspection */
            ...
            if ($this->config[self::CONFIG_PRIMARY_VALUE] == self::CONFIG_PRIMARY_VALUE_EFFECTIVE) {
                ...
                if (!empty($log_events[$profile->meta[Profiler::START_LABEL]])) {
                    foreach ($log_events[$profile->meta[Profiler::START_LABEL]] as $event) {
                        $table .= sprintf(
                            "<tr>%s<td>%d&nbsp;ms</td></tr>",
                            "<td colspan='2'><i>{$event['class']}</i></td>",
                            $event['time'] * 1000
                        );
                    }
                }
        ...
        return sprintf(
            "<h1>Hooks info</h1><div class='tracy-inner'>%s</div>",
            Profiler::isEnabled() ? $table : "Hooks profiling disabled."
        );
    }
                ...
                if (!empty($matches[1])) {
                    ...
                    foreach ($matches as $m) {
                        if (!wa()->getPlugin('plugmein')->getSettings('long_events') || $m['time'] > 0.001) {
                            $events[$hook][] = ['class' => $m['class'], 'method' => $m['method'], 'time' => $m['time']];
                        }
                    }
                }
