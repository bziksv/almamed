...
class shopProduct implements ArrayAccess
{
	...
	public function userValueRelated($limit = 5, $available_only = false, $exclude = array())
    {
        ...
        if(!$related_inc = wa()->getConfig()->getConfigFile('config')['related'])
            $related_inc = 20;

        for($i = 1;$i <= $related_inc;$i++){

            $user_value_related = $this->getData('user_value_related_'.$i);
            // cross selling on (using similar setting for type)
            if ($user_value_related == 1) { //|| $user_value_related === null
                $type = $this->getType();
                if ($type['user_value_related_'.$i]) {
                    $hash = $type['user_value_related_'.$i].($type['user_value_related_'.$i] == 'alsobought' ? '/'.$this->getId() : '');
                    $collection = new shopProductsCollection($hash);
                    if ($type['user_value_related_'.$i] != 'alsobought') {
                        $collection->orderBy('RAND()');
                    }else{
                        continue;
                    }
                ...
                $collection = new shopProductsCollection('related/user_value_related_'.$i.'/'.$this->getId());
            }
