(function ($) {
    ...
    $.products = {
        ...
        /**
         * Custom badges used to be base64(HTML) in the URL → 414 Request-URI Too Large.
         * Keep only short keys: s:* and c:<md5>.
         */
        sanitizeFilterBadgeParam: function(value) {
            if (!value) {
                return value;
            }
            var v = this.normalizeFilterParam(String(value));
            var parts = v.split('|');
            var kept = [];
            for (var i = 0; i < parts.length; i++) {
                ...
                if (/^c:/i.test(p) && !/^c:[a-f0-9]{32}$/i.test(p)) {
                    continue;
                }
                kept.push(p);
            }
        ...
        buildProductsUrlComponent: function (params) {
            ...
            if (params.filter_badge) {
                params.filter_badge = this.sanitizeFilterBadgeParam(params.filter_badge);
                if (!params.filter_badge) {
