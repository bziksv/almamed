...
( function($) {
    ...
    // Синхронизация иконки гамбургера и закрытие поиска при открытии меню
    var syncMobileMenuIcon = function() {
        var btn = document.querySelector('.header-mobile-bar__menu.mobile-nav-button[data-nav="all"]');
        if (!btn) {
            return;
        }
        var icon = btn.querySelector('.header-mobile-bar__menu-icon') || btn.querySelector('.material-icons');
        var isOpen = document.body.classList.contains('nav-all-open');
        if (icon) {
    ...
    var closeMobileSearch = function() {
        var bar = document.getElementById('header-mobile-bar');
        if (!bar || !bar.classList.contains('is-search-open')) {
            return;
        }
        bar.classList.remove('is-search-open');
        var toggle = bar.querySelector('.header-mobile-bar__search-toggle');
        var panel = document.getElementById('header-mobile-search-panel');
        var icon = toggle && toggle.querySelector('.header-mobile-bar__search-icon');
        if (toggle) {
    ...
    var bindEvents = function() {
        ...
        $(document).on('keydown', function(e) {
            if (e.key === 'Escape' && $('body').hasClass('nav-all-open')) {
                hideHiddenMenu();
            }
        });
    ...
    // Узкий desktop/tablet: категории в гамбургер из sidebar (без второго HTML-блока)
    var syncMobileCategoriesFromSidebar = function() {
        if (!window.matchMedia || !window.matchMedia('(max-width: 1200px)').matches) {
            return;
        }
        var $dst = $('.mobile-nav-categories');
        var $src = $('.sidebar-group--categories .base-menu').first();
        if (!$dst.length || !$src.length || $dst.children().length) {
            return;
        }
        $dst.html($src.children().clone());
        $('.mobile-nav-categories.base-menu.type3 .selected').parents('ul').each(function(){
            $(this).show().prev().find('i').html('&#xE5CE;');
        });
        $('.mobile-nav-categories.base-menu.type3 .selected').children('ul').show().prev().find('i').html('&#xE5CE;');
    };

    // Показать / скрыть меню
    var showHiddenMenu = function(el) {
        var nav = $(el).data('nav');
        var cls = 'nav-' + nav + '-open';
        ...
        if (nav === 'all') {
            closeMobileSearch();
            syncMobileCategoriesFromSidebar();
        }
        $("body").addClass(cls);
        syncMobileMenuIcon();
    };
