...
$(document).ready(function () {
	...
	$('.sidebar-group .tags-button > a').click(function(){
		
		$(this).closest('.sidebar-group').find('.tags-params-sidebar').toggleClass(function(i, className, state){
				
					let txtBtn = 'Показать все';
					if(!$(this).hasClass('visible'))
						txtBtn = 'Скрыть все';
				
					$(this).closest('.sidebar-group').find('.tags-button > a').text(txtBtn);	
    ...
    function initProductCarousels(){if(!$.fn.owlCarousel){return false;}$(".product-list.thumbs.carousel:not(.owl-loaded)").each(function(){$(this).owlCarousel({dots:false,items:1,nav:true,navText:["",""],responsive:{0:{items:1},481:{items:2},741:{items:3},993:{items:$('.without-sidebar').length==0?4:3},1281:{items:$('.without-sidebar').length==0?5:4}}});});return true;}
    if(!initProductCarousels()){$(window).on('load',function(){initProductCarousels();});}
	...
	//END Распродажа до конца месяца
