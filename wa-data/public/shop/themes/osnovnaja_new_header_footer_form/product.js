...
$(function () {
    ...
    $(".more-images a").not('#product-image-video').click(function (e) {
        ...
        var buildDisplayCandidates = function(href, size) {
            var match = href.match(/^(.+\.)(\d+(?:x\d+)?)(\.[^./?]+)(\?.*)?$/);
            if (!match) {
                return [href];
            }
            var candidates = [];
            if (size) {
                candidates.push(match[1] + size + match[3] + (match[4] || ''));
                if (size.indexOf('x') === -1) {
                    candidates.push(match[1] + size + 'x0' + match[3] + (match[4] || ''));
                }
            }
        ...
        var applyDisplayImage = function(displaySrc) {
            mainImg.attr('src', displaySrc);
            if (mainPicture.length) {
                var webpSource = mainPicture.find('source[type="image/webp"]');
                var webpSrc = displaySrc.replace(/\.(jpe?g|png)(\?.*)?$/i, '.webp$2');
                if (webpSource.length && webpSrc !== displaySrc) {
                    $('<img>').attr('src', webpSrc).on('load', function() {
                        webpSource.attr('srcset', webpSrc);
                    }).on('error', function() {
                        webpSource.remove();
                    }).each(function() {
                        if (this.complete) {
        ...
        var loadDisplayImage = function(candidates, index) {
            index = index || 0;
            ...
            var candidateSrc = candidates[index];
            var probe = $('<img>').attr('src', candidateSrc);
            probe.on('load', function () {
                applyDisplayImage(candidateSrc);
            }).on('error', function () {
                loadDisplayImage(candidates, index + 1);
            }).each(function() {
                if (this.complete) {
                    if (this.naturalWidth) {
                        $(this).trigger('load');
        ...
        loadDisplayImage(buildDisplayCandidates(thumbHref, displaySize));
